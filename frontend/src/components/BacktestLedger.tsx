...
export const BacktestLedger: React.FC = () => {
    ...
    return (
        <div style={{ background: "#080A0F", borderRadius: 0, border: "none", overflow: "hidden", display: "flex", flexDirection: "column" }}>

            {/* Header */}
            <div style={{ background: "#0B0F19", padding: "1rem 1.25rem", borderBottom: "none", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
                    <Activity size={18} color="var(--accent-cyan)" />
                    <h2 style={{ fontSize: "0.95rem", fontWeight: 800, margin: 0, color: "#E2E8F0", letterSpacing: "0.02em", textTransform: "uppercase" }}>
                ...
                <div style={{ flex: "0 0 160px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ position: "relative", width: "120px", height: "120px" }}>
                        <svg width="120" height="120" viewBox="0 0 120 120">
                            ...
                            <circle
                                ...
                                r="50"
                                fill="none"
                                stroke={stats.win_rate > 50 ? "#10B981" : "#F59E0B"}
                                strokeWidth="12"
                                strokeDasharray="314"
                        ...
                        <div style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                            <span className="mono" style={{ fontSize: "1.5rem", fontWeight: 800, color: "#FFF" }}>{stats.win_rate.toFixed(0)}%</span>
                    ...
                    <div style={{ display: "flex", gap: "1rem", marginTop: "1rem", textAlign: "center" }}>
                        <div>
                            ...
                            <div className="mono" style={{ fontSize: "1rem", fontWeight: 800, color: "#10B981" }}>{stats.wins}</div>
                        </div>
                        <div>
                            <div style={{ fontSize: "0.65rem", color: "var(--text-dim)", textTransform: "uppercase" }}>Losses</div>
                            <div className="mono" style={{ fontSize: "1rem", fontWeight: 800, color: "#F43F5E" }}>{stats.losses}</div>
                        </div>
                    </div>
                </div>
                                ...
                                {stats.active.map((trade: any) => (
                                    <div key={trade.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(255,255,255,0.02)", border: "none", padding: "0.6rem 0.8rem", borderRadius: 0 }}>
                                        <div style={{ display: "flex", alignItems: "center", gap: "0.8rem" }}>
                                            <div style={{ background: "#F59E0B", color: "#000", fontSize: "0.65rem", fontWeight: 800, padding: "0.1rem 0.3rem", borderRadius: 0 }}>PENDING</div>
                                            <span style={{ fontWeight: 800, color: "#FFF", fontSize: "0.85rem" }}>{trade.symbol}</span>
                                        </div>
                                        <div className="mono" style={{ display: "flex", gap: "1rem", fontSize: "0.75rem", color: "var(--text-dim)" }}>
                                            <span><Target size={12} color="#10B981" style={{ marginRight: '4px' }} />{trade.tp_price}</span>
                                            <span><Crosshair size={12} color="#06B6D4" style={{ marginRight: '4px' }} />{trade.entry_price}</span>
                                            <span><ShieldAlert size={12} color="#F43F5E" style={{ marginRight: '4px' }} />{trade.sl_price}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
