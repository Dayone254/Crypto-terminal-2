...
def normalize_features(features: FeatureDict, direction: str) -> dict[str, float]:
    ...
    rs_vs_btc = float(features.get("rs_vs_btc") or 0.0)
    ...
    if direction == "LONG":
        ...
        norm["relative_strength"] = max(-1.0, min(1.0, rs_vs_btc / 3.0))
        ...
        norm["relative_weakness"] = max(-1.0, min(1.0, -rs_vs_btc / 3.0))
...
def score(
    ...
    # Extract weights based on direction
    weights = config.component_weights.LONG if trade_direction == "LONG" else config.component_weights.SHORT
    
    # Base weighted sum [-1.0, 1.0] -> scaled to [-50, +50] to map to [0, 100] max
    weighted_sum = sum(norm.get(k, 0.0) * w for k, w in weights.items())
    base_calc = weighted_sum * 50.0  # Convert to +/- points
    ...
    if (norm.get("rsi_oversold", 0.0) > 0.5 or norm.get("rsi_overbought", 0.0) > 0.5) and norm.get("fib_confluence", 0.0) > 0.5:
        interactions += config.interaction_bonuses.confluence_bonus
    ...
    # 7. Relative Strength (BTC Beta)
    rs = features.get("rs_vs_btc", 0.0)
    if rs > 2.0:
        if trade_direction == "LONG":
            bonus = min(15.0, rs * 2.5) # Max 15 pt bonus for extreme relative outperformance
            interactions += bonus
            components_dump["rs_btc_bonus"] = bonus
        else:
            penalty = min(15.0, rs * 2.5)
            interactions -= penalty
            components_dump["rs_btc_short_penalty"] = -penalty
    elif rs < -2.0:
        if trade_direction == "SHORT":
            bonus = min(15.0, abs(rs) * 2.5)
            interactions += bonus
            components_dump["rs_btc_weakness_bonus"] = bonus
        else:
            penalty = min(15.0, abs(rs) * 2.5)
            interactions -= penalty
            components_dump["rs_btc_long_penalty"] = -penalty

    total = baseline + base_calc + interactions
